...
def tailscale_webhook():
    ...
    try:
        ...
        if not payload:
            logger.warning("API INGEST - Tailscale webhook sent an empty payload.")
        ...
        logger.info("API INGEST - Tailscale Notification created; ticket_number=%s", ticket_number)
        ...
        try:
            ...
            logger.info("API INGEST - Ticket %s created and notifications sent.", ticket_number)
        except Exception:
            logger.exception("API INGEST - Failed to send ticket status notifications for %s", ticket_number)

        return jsonify({"status": "success", "ticket": ticket_number}), 200

    except Exception:
        cid = uuid.uuid4().hex[:8]
        logger.exception("API INGEST - Tailscale webhook processing failed; correlation_id=%s", cid)
...
def uptime_kuma_webhook():
    ...
    try:
        if not request.is_json:
            logger.warning("API INGEST - Uptime-Kuma webhook sent invalid content type.")
            return jsonify({"error": "Invalid content type"}), 400
        payload = request.json
        logger.info("API INGEST - Uptime Kuma webhook received; source=%s", _opaque_id(payload.get("monitor", {}).get("name")))
        ...
        if status not in [0, 2]:
            logger.info("API INGEST - Skipping ticket creation for monitor=%s (status=%s)", _opaque_id(monitor_name, "monitor"), status_text)
        ...
        logger.info("API INGEST - Uptime-Kuma Notification %s created (status=%s)", ticket_number, status_text)
        ...
        try:
            ...
            logger.info("API INGEST - Ticket %s status notifications sent.", ticket_number)
        except Exception:
            logger.exception("API INGEST - Failed to send ticket status notifications for %s", ticket_number)
        ...
        cid = uuid.uuid4().hex[:8]
        logger.exception("API INGEST - Uptime Kuma webhook processing failed; correlation_id=%s", cid)
        return jsonify({"error": "Internal server error"}), 500
"""
...
def librenms_webhook():
    ...
    try:
        if not request.is_json:
            logger.warning("API INGEST - LibreNMS webhook sent invalid content type.")
            return jsonify({"error": "Invalid content type"}), 400

        payload = request.json
        if not payload:
            logger.warning("API INGEST - LibreNMS webhook sent an empty payload.")
            return jsonify({"error": "Empty payload"}), 400

        logger.info("API INGEST - LibreNMS webhook received; source=%s", _opaque_id(payload.get("hostname")))
        ...
        if state != 1:
            logger.info("API INGEST - Skipping ticket creation for host=%s (state=%s)", _opaque_id(hostname, "host"), state)
        ...
        logger.info("API INGEST - LibreNMS Notification %s created (severity=%s)", ticket_number, severity or 'unknown')
        ...
        cid = uuid.uuid4().hex[:8]
        logger.exception("API INGEST - LibreNMS webhook processing failed; correlation_id=%s", cid)
        return jsonify({"error": "Internal server error"}), 500